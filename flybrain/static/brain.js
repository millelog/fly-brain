// 3D brain: soma point cloud with activity heat, clip slabs, per-class opacity, picking + inspector, neuropil hulls.
...
const HOT = [1.0, 0.85, 0.35], roColors = ['#4ade80', '#f472b6', '#60a5fa', '#facc15', '#fb923c'];
let meta, send, N, V, vert, v2n, pos, base, col, heat, alpha, last, renderer, scene, camera, controls, geo, points, colorAttr, planes, cx, cy, cz, bbox;
...
export function init(o) {
  ...
  base = new Float32Array(V * 3); col = new Float32Array(V * 4); heat = new Float32Array(V); alpha = new Float32Array(V).fill(1); last = new Float32Array(V).fill(-1e9);
  paintBase();
  ...
  points = new THREE.Points(geo, new THREE.PointsMaterial({ size: 5, vertexColors: true, sizeAttenuation: true, transparent: true, depthWrite: false, clippingPlanes: planes }));
  scene.add(points);
  colorAttr = geo.getAttribute('color');
  addEventListener('resize', resize); resize();
  buildPanel(); loadHulls();
...
export function resize() { const el = $('right'); renderer.setSize(el.clientWidth, el.clientHeight, false); camera.aspect = el.clientWidth / el.clientHeight; camera.updateProjectionMatrix(); }
...
function buildPanel() {
  ...
  for (const o of $('ro').options) o.selected = ['shiu_mn9', 'dna02_l', 'dna02_r', 'dnp09'].includes(o.value);
  $('pokepop').value = 'shiu_sugar';
  ...
  $('opac').oninput = e => { opac[e.target.dataset.c] = +e.target.value; $('ov_' + e.target.dataset.c).textContent = e.target.value; };
...
async function onPick(e) {
  ...
  const hit = ray.intersectObject(points).find(h => alpha[h.index] > 0.05 && planes.every(p => p.distanceToPoint(new THREE.Vector3().fromBufferAttribute(geo.attributes.position, h.index)) >= 0));
  if (!hit) return;
...
export function onFrame(t, idx, cnt) {
  ...
  for (let k = 0; k < idx.length; k++) { const v = vert[idx[k]]; if (v >= 0) { heat[v] = Math.min(1, heat[v] + 0.4 * cnt[k]); last[v] = t; } const tr = traces.get(idx[k]); if (tr) tr[199] = cnt[k]; }
...
function readoutRates() {
  const sel = [...$('ro').selectedOptions].map(o => o.value);
  const tot = new Map(sel.map(s => [s, 0])), sets = new Map(sel.map(s => [s, new Set(meta.populations[s])]));
  ...
  let sr = '';
  if (selected != null) { let c = 0; for (const f of hist) for (let k = 0; k < f.idx.length; k++) if (f.idx[k] === selected) c += f.cnt[k]; sr = ` · ${(c * 1000 / span).toFixed(0)} Hz`; }
  $('ititle').textContent = $('ititle').textContent.replace(/ · \d+ Hz$/, '') + sr;
...
function stats() {
  ...
  $('stats').textContent = `${meta.device}${meta.replay ? ' · replay' : ''} · ${N.toLocaleString()} neurons, ${meta.no_soma.toLocaleString()} without soma (hidden)\nbio ${(bioT / 1000).toFixed(2)} s · wall ${wall.toFixed(0)} s · ${(wall / ((bioT - bioT0) / 1000 || 1)).toFixed(1)} s/bio-s\nactive last frame: ${active}`;
}
function drawTraces() {
...
// --- render loop: decay heat, blend color and alpha, update view-aligned slab ---
const dir = new THREE.Vector3();
function loop() {
  requestAnimationFrame(loop);
  const actOnly = $('actonly').checked, iso = inSet;
  for (let v = 0; v < V; v++) {
    const h = heat[v]; heat[v] = h * 0.9; const i = v2n[v];
    col[4 * v] = base[3 * v] + h * (HOT[0] - base[3 * v]); col[4 * v + 1] = base[3 * v + 1] + h * (HOT[1] - base[3 * v + 1]); col[4 * v + 2] = base[3 * v + 2] + h * (HOT[2] - base[3 * v + 2]);
    let a = opac[meta.super_class[i]] ?? 1;
    if (actOnly) a *= Math.max(h, bioT - last[v] < 300 ? 0.15 : 0.02);
    if (iso && !iso.has(i)) a *= 0.02;
    col[4 * v + 3] = alpha[v] = a * 0.9;
  }
